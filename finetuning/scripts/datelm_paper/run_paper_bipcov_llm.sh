...
# 2) Train embeddings (LLM hidden states; shared)
if [[ ! -f "${EMB_TRAIN_DIR}/train_emb.npy" ]] || ! grep -q '"train_complete": true' "${EMB_TRAIN_DIR}/progress.json" 2>/dev/null; then
  echo "[2/7] Computing LLM train embeddings -> ${EMB_TRAIN_DIR}/train_emb.npy"
...
# 3) Ref embeddings (per task)
if [[ ! -f "${EMB_REF_DIR}/ref_emb.npy" ]]; then
  echo "[3/7] Computing ${TASK} ref embeddings -> ${EMB_REF_DIR}/ref_emb.npy"
  PYTHONUNBUFFERED=1 "${PY_TRAIN_EVAL}" "${PROJ_ROOT}/finetuning/scripts/datelm_paper/compute_llm_last_token_embeddings.py" \
    --datelm_root "${DATELM_ROOT}" \
    --model_name "${BASE_MODEL}" \
    --no_train \
...
# 4) BipCov scoring (greedy max coverage)
if [[ ! -f "${METRICS_NPY}" ]]; then
  echo "[4/7] BipCov(LLM emb) scoring -> ${METRICS_NPY}"
  PYTHONUNBUFFERED=1 "${PY_TRAIN_EVAL}" "${DATELM_ROOT}/methods/bipcov/probe_bipcov_from_emb.py" \
    ...
    --device cuda \
    --batch_rows 50000 \
    --save_selected "${SCORES_DIR}/${METHOD}_selected_indices.json" \
