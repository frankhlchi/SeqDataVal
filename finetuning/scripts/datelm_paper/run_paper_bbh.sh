...
BBH_DIR="${DATELM_ROOT}/data/eval/bbh"
BBH_TASKS_DIR="${BBH_DIR}/bbh"
BBH_REF_JSONL="${BBH_DIR}/${RUN_TAG}_bbh_ref_100_promptlabel.jsonl"

SELECT_DIR="${DATELM_ROOT}/selected_data/${RUN_TAG}/bbh"
SCORES_DIR="${DATELM_ROOT}/scores/${RUN_TAG}/bbh"
EMB_TRAIN_DIR="${DATELM_ROOT}/embeddings/${RUN_TAG}/tulu3_train_bge"
EMB_REF_DIR="${DATELM_ROOT}/embeddings/${RUN_TAG}/bbh_ref_bge"

ADAPTER_DIR="${DATELM_ROOT}/checkpoints/${RUN_TAG}_${METHOD}_bbh_lora"
MERGED_DIR="${DATELM_ROOT}/checkpoints/${RUN_TAG}_${METHOD}_bbh_merged"
RESULTS_DIR="${DATELM_ROOT}/results/${RUN_TAG}_${METHOD}_bbh_official"
...
if [[ "${METHOD}" == "random" ]]; then
  ...
  if [[ ! -f "${BBH_REF_JSONL}" ]]; then
    # Align BipCov ref with DATE-LM baselines: use DATASETS[bbh].get_all_test_prompts(prompt_only=False).
    echo "  - Preparing BBH ref JSONL (DATASETS; prompt+label) -> ${BBH_REF_JSONL}"
    "${PY_TRAIN_EVAL}" "${PROJ_ROOT}/finetuning/scripts/datelm_paper/prepare_ref_jsonl_from_datasets.py" \
      --datelm_root "${DATELM_ROOT}" \
      --task bbh \
      --out_jsonl "${BBH_REF_JSONL}" \
      --num_samples 100 \
      --seed "${SEED_REF}" \
      > "${LOG_DIR}/${RUN_TAG}_bipcov_bbh_prepare_ref.log" 2>&1
  else
    echo "  - BBH ref JSONL exists: ${BBH_REF_JSONL}"
  fi

  if [[ ! -f "${EMB_REF_DIR}/ref_emb.npy" ]]; then
    echo "  - Computing BBH ref embeddings -> ${EMB_REF_DIR}/ref_emb.npy"
    "${PY_EMB}" "${PROJ_ROOT}/finetuning/compute_embeddings_for_datelm.py" \
      --ref_jsonl "${BBH_REF_JSONL}" \
      --ref_max_samples 100 \
      --out_dir "${EMB_REF_DIR}" \
      --no_train \
      --model "BAAI/bge-large-en-v1.5" \
...
if [[ ! -f "${ADAPTER_DIR}/adapter_config.json" ]]; then
  ...
  PYTHONUNBUFFERED=1 "${PY_TRAIN_EVAL}" "${PROJ_ROOT}/finetuning/scripts/datelm_paper/train_lora_hf_paper.py" \
    ...
    > "${LOG_DIR}/${RUN_TAG}_${METHOD}_bbh_finetune.log" 2>&1
else
...
# 5) Eval (BBH)
echo "[5/6] Running official BBH eval -> ${RESULTS_DIR}"
cd "${DATELM_ROOT}"
"${PY_TRAIN_EVAL}" -m minimal_multitask.eval.bbh.run_eval \
  ...
  > "${LOG_DIR}/${RUN_TAG}_${METHOD}_bbh_eval.log" 2>&1
